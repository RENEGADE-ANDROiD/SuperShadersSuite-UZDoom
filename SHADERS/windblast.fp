...
Material ProcessMaterial()
{
    ...
    Material material;
	vec4 gettx = getTexel(texCoord);
	vec4 getty = getTexel(texCoord + vec2(timer,0));
	vec4 gettz = getTexel((texCoord * 2.5) + vec2(timer * 1.2,0));
	
	float process = (getty.y * gettz.y)*2 * gettx.x;
	
    material.Base = vec4(process,process,process,1);
