...
float GetWarpMX()
{
    return float(warpMultX);
}

float GetWarpMY()
{
    return float(warpMultY);
}

float GetHardScan()
{
    return -float(CRThardScan);
}

vec2 GetIResolution()
{
    return vec2(textureSize(InputTexture, 0));
}

vec2 GetRes()
{
    return (GetIResolution().xy / 3.0);
}
...
{
    pos = floor(pos * GetRes() + off) / GetRes();
...
float Scan(vec2 pos,float off)
{
    ...
    return Gaus(dst + off, GetHardScan());
}
...
vec2 Warp(vec2 pos)
{
    if (warpEnable != 0)
    {
        warp = vec2(1.0 / GetWarpMX(), 1.0 / GetWarpMY()); 
    }
...
void main()
{
    ...
    if (VHSEnable != 0)
    {
        ...
        float one_y = GetIResolution().y / linesN; //field line
        col = nn(floor(uv*GetIResolution().xy/one_y)*one_y);
