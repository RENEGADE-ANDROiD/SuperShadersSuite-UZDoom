...
Material ProcessMaterial()
{
    ...
    material.Base = getTexel(texCoord + GetReflectionVector()) + addEnv;
    material.Normal = GetBumpedNormal(tbn, texCoord + GetReflectionVector()) + GetBumpedNormal(tbn, texCoord * 0.5 + -GetReflectionVector()) + GetBumpedNormal(tbn, texCoord * 0.15 + GetReflectionVector());
	material.Normal = material.Normal / 3;
	
	
#if defined(SPECULAR)
    material.Specular = texture(speculartexture, texCoord + GetReflectionVector()).rgb;
...
{
    vec2 texCoord = vTexCoord.st;
	return texture(watnoise, texCoord + (timer * 0.02)).xy * 0.2;
}
